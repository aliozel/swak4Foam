...
)
{
    ...
    if(
        this->autowrite_
//_AO-10/10/2014- not working for post-processing tool (particleStressModelling)
//		&&
//		this->obr_.time().outputTime()
//_AO
    ) {
		Info << " WARNING !!! This subroutine, expressionField.C in swakFunctionObjects,  was modified by A. Ozel " << endl;        
		field_->write();
    }
}
